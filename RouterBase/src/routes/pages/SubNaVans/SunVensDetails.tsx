...
function SubVensDetails() {
    ...
    return (  
        ...
        <React.Fragment>
            <Link to=".." 
            relative="path"
            className=" w-fit before:content-[''] relative before:absolute  before:left-0 before:bottom-0  before:h-[2px] before:w-0  before:rounded before:bg-slate-400 flex items-center font-pop before:transition-all ease-in-out  ml-2 hover:before:w-[10rem]">
            <BiChevronLeft className="text-xl   "/>
            Back To All Vans
            </Link>
        {Vens.filter(v => v.id === parseInt(HotedVan.id!)).map(v => (
            <div className=" flex-col  bg-white rounded-md p-3 m-5 shadow-2xl flex  gap-3 font-inter w-[496px] h-max" key={v.id}>
                <div className="flex">
